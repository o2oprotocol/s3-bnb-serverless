...
![Select Cognito Service screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/select-cognito-service.png)

Select **Manage Federated Identities**.

![Select Manage Federated Identities Screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/select-manage-federated-identities.png)
...
![Fill Cognito Identity Pool Info Screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/fill-identity-pool-info.png)

Select **Authentication providers**. Under **Cognito** tab, enter **User Pool ID** and **App Client ID** of the User Pool created in the [Create a Cognito user pool]({{ site.baseurl }}{% link _chapters/create-a-cognito-user-pool.md %}) chapter. Select **Create Pool**.

![Fill Authentication Provider Info Screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/fill-authentication-provider-info.png)
...
![Select Edit Policy Document Screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/select-edit-policy-document.png)

It will warn you to read the documentation. Select **Ok** to edit.

![Select Confirm Edit Policy Screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/select-confirm-edit-policy.png)

<img class="code-marker" src="{{ site.baseurl }}/assets/s.png" />Add the following policy into the editor. Replace `YOUR_S3_UPLOADS_BUCKET_NAME` with the **bucket name** from the [Create an S3 bucket for file uploads]({{ site.baseurl }}{% link _chapters/create-an-s3-bucket-for-file-uploads.md %}) chapter. And replace the `YOUR_API_GATEWAY_REGION` and `YOUR_API_GATEWAY_ID` with the ones that you get after you deployed your API in the last chapter.
...
![Submit Cognito Identity Pool Policy Screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/submit-identity-pool-policy.png)
...
![Cognito Identity Pool Created Screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/identity-pool-created.png)

Take a note of the **Identity pool ID** which will be required in the later chapters.

![Cognito Identity Pool Created Screenshot]({{ site.baseurl }}/assets/cognito-identity-pool/identity-pool-id.png)
